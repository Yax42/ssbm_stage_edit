	...
	public class MainForm : Form
	{
		...
		private void Import(int idx, string impPath)
		{
			...
			if (impPath.Length != 0)
			{
				...
				{
					using (FileStream fileStream = new FileStream(impPath, FileMode.Open, FileAccess.Read, FileShare.Read))
					using (BinaryReader binaryReader = new BinaryReader(fileStream, Encoding.Default))
					using (FileStream fileStream2 = new FileStream(this.imgPath, FileMode.Open, FileAccess.Write, FileShare.None))
					using (BinaryWriter binaryWriter = new BinaryWriter(fileStream2, Encoding.Default))
					{
